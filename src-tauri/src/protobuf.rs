...
pub fn pb_read_varint(data: &[u8], offset: usize) -> Result<(u64, usize), String> {
    let mut result = 0u64;
    let mut shift = 0;
    let mut pos = offset;
...
/// Skip Protobuf Field
pub fn pb_skip_field(data: &[u8], offset: usize, wire_type: u8) -> Result<usize, String> {
    match wire_type {
        0 => {
            // Varint
            let (_, new_offset) = pb_read_varint(data, offset)?;
            Ok(new_offset)
        }
        1 => {
            // 64-bit
        ...
        2 => {
            // Length-delimited
            let (length, content_offset) = pb_read_varint(data, offset)?;
...
pub fn remove_field(data: &[u8], field_num: u32) -> Result<Vec<u8>, String> {
    ...
    while offset < data.len() {
        ...
        let (tag, new_offset) = pb_read_varint(data, offset)?;
        let wire_type = (tag & 7) as u8;
        let current_field = (tag >> 3) as u32;

        if current_field == field_num {
            // Skip this field
            offset = pb_skip_field(data, new_offset, wire_type)?;
        } else {
            // Keep other fields
            let next_offset = pb_skip_field(data, new_offset, wire_type)?;
...
pub fn pb_find_field(data: &[u8], target_field: u32) -> Result<Option<Vec<u8>>, String> {
    let mut offset = 0;

    while offset < data.len() {
        let (tag, new_offset) = match pb_read_varint(data, offset) {
            Ok(v) => v,
        ...
        if field_num == target_field && wire_type == 2 {
            let (length, content_offset) = pb_read_varint(data, new_offset)?;
