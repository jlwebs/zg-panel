...
export async function request<T>(cmd: string, args?: any): Promise<T> {
  ...
  if (isTauri) {
    ...
    if (mapping) {
      // Mapped command → use api_get/api_post/api_delete to forward to Go backend
      const { invoke } = await import('@tauri-apps/api/core');

      let url = mapping.url;
      let bodyArgs = args ? { ...args } : undefined;

      // Replace path parameters like :accountId
      if (args) {
      ...
      try {
        ...
        if (mapping.method === 'GET') {
          ...
          resultStr = await invoke<string>('api_get', { path: url, port });
        } else if (mapping.method === 'DELETE') {
          const bodyJson = bodyArgs && Object.keys(bodyArgs).length > 0
            ? JSON.stringify(bodyArgs)
            : undefined;
          const config = await loadConfig();
          const port = config?.proxy?.port || 8741;
          resultStr = await invoke<string>('api_delete', { path: url, bodyJson, port });
        } else {
          // POST or PATCH
          ...
          const config = await loadConfig();
          const port = config?.proxy?.port || 8741;
          resultStr = await invoke<string>('api_post', { path: url, bodyJson, port });
        }

        // Parse JSON response
        if (!resultStr || resultStr === '') return null as unknown as T;
        try {
        ...
        console.error(`Tauri API Proxy Error [${cmd}] -> ${mapping.method} ${url}:`, error);
        throw error;
      }
    } else {
      // Unmapped command → direct Rust invoke (e.g. import_from_ide, check_docker, etc.)
      try {
        const { invoke } = await import('@tauri-apps/api/core');
