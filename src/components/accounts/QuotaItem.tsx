...
export function QuotaItem({ label, percentage, resetTime, isProtected, className, Icon }: QuotaItemProps) {
    ...
    return (
        <div className={cn(
            "relative h-[18px] flex items-center px-1 rounded overflow-hidden border border-gray-100/50 dark:border-white/5 bg-gray-50/30 dark:bg-white/5",
            className
        )}>
            ...
            <div className="relative z-10 w-full flex items-center text-[9px] font-mono leading-none gap-1">
                {/* Model Name */}
                <span className="flex-1 min-w-0 text-gray-500 dark:text-gray-400 font-bold truncate text-left flex items-center gap-0.5" title={label}>
                    {Icon && <Icon size={10} className="shrink-0" />}
                    {label}
                </span>

                {/* Reset Time */}
                <div className="w-[50px] flex justify-start shrink-0">
                    {resetTime ? (
                ...
                {/* Percentage */}
                <span className={cn("w-[26px] text-right font-bold transition-colors flex items-center justify-end gap-0.5 shrink-0", getTextColorClass(percentage))}>
                    {isProtected && (
                        <span title={t('accounts.quota_protected')}><Lock className="w-2 h-2 text-amber-500" /></span>
                    )}
                    {percentage}%
                </span>
            </div>
