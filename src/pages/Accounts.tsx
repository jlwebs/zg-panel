...
function Accounts() {
  ...
  const [applyToIDEConfirmId, setApplyToIDEConfirmId] = useState<string | null>(null);
    ...
    try {
      ...
      if (isBatch) {
        ...
        if (msg) {
          ...
          showToast(
            t("accounts.warmup_all_triggered"),
            "success",
          );
        }
      }
  ...
  const ITEMS_PER_PAGE = useMemo(() => {
    ...
    // 回退到原有的动态计算逻辑
    if (!containerSize.height) return viewMode === "grid" ? 6 : 10;
    ...
    if (viewMode === "list") {
      ...
      const cardHeight = 140; // Compact AccountCard height (with spacing)
      const gap = 12; // gap-3

      // Match AccountGrid: grid-cols-2 / md:grid-cols-3
      let cols = 2;
      if (containerSize.width >= 600) cols = 3;
  ...
  const handleApplyToIDE = async (accountId: string) => {
    setApplyToIDEConfirmId(accountId);
  };

  const executeApplyToIDE = async () => {
    if (!applyToIDEConfirmId) return;
    const accountId = applyToIDEConfirmId;
    setApplyToIDEConfirmId(null);
    try {
      await applyToIDE(accountId);
      showToast(t("common.success"), "success");
    } catch (error) {
      showToast(`${t("common.error")}: ${error}`, "error");
    }
  ...
  const handleCopyToken = (accountId: string) => {
    try {
      // Get token synchronously from local state to ensure we
      // do not lose the browser's "transient user activation" (NotAllowedError)
      const account = accounts.find((a) => a.id === accountId);
      // Try multiple possible paths where token might be stored
      const token = account?.token?.refresh_token || (account as any)?.refresh_token;

      if (token) {
        copyToClipboard(token).then((success) => {
          if (success) {
            showToast(t('accounts.copy_token_success'), 'success');
          } else {
            showToast(t('accounts.copy_token_fail', { error: 'Permission denied (Need user gesture)' }), 'error');
          }
        });
      } else {
        showToast(t('accounts.no_token_found'), 'warning');
      }
    } catch (error: any) {
      console.error('Copy token failed:', error);
      showToast(t('accounts.copy_token_fail', { error: error?.message || String(error) }), 'error');
    }
  };
    ...
    <div className="h-full flex flex-col p-5 gap-4 max-w-7xl mx-auto w-full">
      ...
      <div className="flex-none flex items-center gap-2">
        ...
        <div className="flex gap-0.5 bg-gray-100/80 dark:bg-base-200 p-1 rounded-xl border border-gray-200/50 dark:border-white/5 shrink-0">
          ...
          >
            <span className="inline">{t('accounts.all')}</span>
            <span className={cn(
          ...
          >
            <span className="inline">{t('accounts.pro')}</span>
            <span className={cn(
              "px-1.5 py-0.5 rounded-md text-[10px] font-bold transition-colors",
              filter === 'pro'
                ? "bg-blue-100 dark:bg-blue-500/20 text-blue-600 dark:text-blue-400"
          ...
          >
            <span className="inline">{t('accounts.ultra')}</span>
            <span className={cn(
              "px-1.5 py-0.5 rounded-md text-[10px] font-bold transition-colors",
              filter === 'ultra'
                ? "bg-blue-100 dark:bg-blue-500/20 text-blue-600 dark:text-blue-400"
          ...
          >
            <span className="inline">{t('accounts.free')}</span>
            <span className={cn(
              "px-1.5 py-0.5 rounded-md text-[10px] font-bold transition-colors",
              filter === 'free'
                ? "bg-blue-100 dark:bg-blue-500/20 text-blue-600 dark:text-blue-400"
          ...
          <div className="h-full bg-white dark:bg-base-100 rounded-2xl shadow-sm border border-gray-100 dark:border-base-200 flex flex-col overflow-hidden">
            <div className="flex-1 overflow-y-auto">
              <AccountTable
                ...
                ideActiveEmail={ideActiveEmail}
                onApplyToIDE={handleApplyToIDE}
              />
            </div>
          </div>
        ) : (
      ...
      {/* Minimalist Pagination - Floating style */}
      ...
      <ModalDialog
        ...
        title={
          selectedIds.size > 0
            ? t("accounts.dialog.batch_warmup_title")
            : t("accounts.dialog.warmup_all_title")
        }
        message={
          selectedIds.size > 0
            ? t(
        ...
        type="confirm"
        confirmText={t("accounts.warmup_now")}
        isDestructive={false}
      ...
      <ModalDialog
        isOpen={!!applyToIDEConfirmId}
        title="移动至 IDE"
        message={`确定要将账号 ${accounts.find(a => a.id === applyToIDEConfirmId)?.email} 同步到 Antigravity IDE 吗？这将会重启 IDE 并覆盖其当前登录状态。`}
        type="confirm"
        confirmText="立即同步"
        onConfirm={executeApplyToIDE}
        onCancel={() => setApplyToIDEConfirmId(null)}
      />
    </div>
  );
}
